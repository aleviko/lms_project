...
class LessonForm(forms.ModelForm):
    ...
    preview = forms.CharField(widget=Textarea(attrs={'placeholder': 'Описание содержания урока',
                              'rows': 20, 'cols': 35}),
                              label='Описание', required=True)  # В видео на 16:59 required=True нет
    ...
    error_css_class = 'error_field'  # HTML class для полей, не прошедших валидацию для формы на {{ form.as_p }}, в SCC можно расцветить
    ...
    def clean_preview(self):  # переопределение метода проверки для одного поля
        ...
        if len(preview_data) > 200:  # вообще это все равно говнокод: куча констант в коде, связанных с БД только памятью разраба... надеюсь, это  только для "развлечения" обучаемых
